...
namespace PubPlaza.Data
{
    ...
    {
        public static void Seed(PubPlazaContext pubPlazaContext)
        {
            pubPlazaContext.Database.EnsureCreated();//if db is not exist ,it will create database .but ,do nothing .
            if (!pubPlazaContext.Categories.Any())
            {
                pubPlazaContext.Categories.AddRange(Categories.Select(c => c.Value));
            }

            if (!pubPlazaContext.Drinks.Any())
            {
                pubPlazaContext.AddRange
                (
                    new Drink
                    {
                        DrinkName = "Beer",
                        ...
                        ImageThumbnailUrl = "~/images/Drink1.jpg"
                    },
                    new Drink
                    {
                        DrinkName = "Rum & Coke",
